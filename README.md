...
ブラウザの上でそのままコードを書いて実行できる。全8モジュール・68レッスンを順番に進めるだけで、変数からクラス・非同期処理、DOM、TypeScript、React まで——個人開発を始められるところまで進める。専門用語はレッスン本文中のタップでその場に定義が出て、用語集ページで検索もできる。

## 機能

- **68レッスン / 8モジュール** — 土台 → 一歩深く → データを自在に → 設計とモダンJS → ブラウザとDOM → TypeScript → React / Next.js → 個人開発の実践。各レッスンは「読む → 例を読む → 自分で書く → 実行してクリア判定」の流れ
